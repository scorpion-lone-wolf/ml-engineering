...
**Sub-module DL-1b — ML Strategy: Diagnosing and Improving a Model** (a named, non-skippable sub-module, not just implied):
...
**Sub-module DL-2b — Model Interpretability for CV** (named explicitly, not left implicit):
...
**Deliverable:** implement RoPE, grouped-query attention, and a minimal Mixture-of-Experts layer from scratch (NumPy/PyTorch), then explain — in the from-scratch code — exactly what each replaces from the original DL-3 Transformer and why.
...
|---|---|---|---|
| DL-1 (+ DL-1b ML Strategy) | **Tabular/image classifier with a full error-analysis writeup** — e.g., a defect/fraud/quality classifier — trained multiple ways (different init, regularization, optimizers) with a documented bias/variance diagnosis at each iteration | Every ML team does this triage constantly before reaching for a bigger model | Reading training curves correctly, deciding *what to fix next* instead of guessing |
...
After Stages DL-0 through DL-6 are substantially mastered, build **2–3 portfolio-grade production DL systems** that go beyond the stage-by-stage showcase projects above — these are the final, polished, end-to-end systems for your GitHub/resume. Each must be end to end.
...
6. **Build the Complete Roadmap** — show Stages DL-0 through DL-6 (including sub-modules DL-1b, DL-2b, DL-3b) at a high level: purpose, major topics, practical deliverable, mastery checkpoint, dependency on prior stages.
